...
> [!NOTE] このページは **HackMD 公開用のポータブル版**です。
> GitHub 上の元ファイルは
> [`docs/d6e-overview-ja.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/d6e-overview-ja.md)
> にあり、本ファイルとはリンクの解決方法だけが異なります(本ページのリンクはすべて
> GitHub の絶対 URL に書き換え済み)。Discord 等に共有する際は、本 HackMD ページの URL
> を貼ってください(Discord 本文上では Mermaid 図はレンダリングされないため)。
...
詳細は [`docs/architecture.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/architecture.md) の Sequence 節と
[`src/lib/server/oauth.ts`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/lib/server/oauth.ts) /
[`src/lib/server/session.ts`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/lib/server/session.ts) を参照。

### 3.2 必要な環境変数(本リポジトリ [`.env.example`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/.env.example) より)
...
[`docs/d6e-api-integration.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/d6e-api-integration.md) に網羅されています。

- **ファイルアップロード**:`POST /api/v1/workspaces/{wsId}/files/multipart`(Rust API 側)
  - 本アプリでは [`src/routes/api/upload/+server.ts`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/routes/api/upload/+server.ts) が
    `multipart/form-data` をそのまま中継。
- **自然言語ワークフロー実行**:`POST /api/workflows/execute-by-intent`(SvelteKit Frontend)
  - 本アプリでは [`src/routes/api/intent/+server.ts`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/routes/api/intent/+server.ts) が呼び出し。
  - LLM が ` ```json ... ``` ` ブロックで返す
    [`Journal` Zod スキーマ](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/lib/journal-schema.ts) を契約として使う(後述)。
- **チャットセッション CRUD**:`GET / POST / PATCH / DELETE /api/chat-sessions[/{id}]`(SvelteKit Frontend)
  - 本アプリでは
    [`src/routes/api/chat-sessions/+server.ts`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/routes/api/chat-sessions/+server.ts) と
    [`src/routes/api/chat-sessions/[id]/+server.ts`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/src/routes/api/chat-sessions/%5Bid%5D/+server.ts)
...
- **ワークスペースプロンプトルール登録**:`POST /api/workspace-prompt-rules`(SvelteKit Frontend)
  - LLM の挙動契約を 1 度だけ書き込む。本アプリは
    [`scripts/init-workspace.mjs`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/scripts/init-workspace.mjs) から `npm run init` で実行。
...
[`d6e-ai-keiri-example`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/README.md) があります。日本の経理業務(レシート → 仕訳)を
...
- [`docs/architecture.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/architecture.md) - シーケンス図 / 信頼境界 / モジュール責務一覧
- [`docs/d6e-api-integration.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/d6e-api-integration.md) - 全エンドポイントの request/response 仕様
- [`docs/workspace-setup.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/workspace-setup.md) - ワークスペースの初期セットアップ手順
- [`docs/llm-output-contract.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/llm-output-contract.md) - LLM 出力の JSON 契約(Zod パース + raw 表示フォールバック)
- [`docs/migration-to-full-integration.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/migration-to-full-integration.md) - モックから本番統合への移行ガイド
...
| 自然言語起点での起動            | **`execute-by-intent`** + **Workspace Prompt Rule**                            | "intent" を Workflow にルーティングし、LLM に決まった JSON 契約で結果を返させる([`docs/llm-output-contract.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/docs/llm-output-contract.md))      |
...
- CI から自動更新する場合は、本リポジトリの
  [`scripts/init-workspace.mjs`](https://github.com/d6e-ai/d6e-ai-keiri-example/blob/main/scripts/init-workspace.mjs) と同じパターン
...
詳細な設計議論が必要になった場合は、本ドキュメントとは別途
[GitHub 上の `.plans/*.plan.md`](https://github.com/d6e-ai/d6e-ai-keiri-example/tree/main/.plans)
を起こして進める想定です。
