...
- `skills/<skill-name>/SKILL.md` 形式で 3 つの Agent Skill を同梱している (`d6e-auth-integration` / `d6e-workspace-api-client` / `d6e-prompt-driven-ui`)。インストールは `npx skills add d6e-ai/d6e-custom-frontend-skills --skill <name>` で行う (GitHub の `owner/repo` 省略記法を利用できる)。
