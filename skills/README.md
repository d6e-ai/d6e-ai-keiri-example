...
| [`d6e-workspace-api-client`](./d6e-workspace-api-client/SKILL.md) | The server-side proxy layer (`/api/upload`, `/api/intent`, `/api/chat-sessions`), the `caller + accessToken + AbortSignal` wrapper convention in `src/lib/server/d6e-client.ts`, the Bearer vs `auth-token` cookie header matrix, workspace SQL / files / workflows / members / documents / embeddings / pinned-charts, the **async intent job API** (create / poll / cancel for long-running agent runs that exceed Vercel's `maxDuration`), the SHA-256-keyed idempotent prompt-rule bootstrap in `scripts/init-workspace.mjs`, and the optional **Drive Sync mirror endpoints** + **workspace pending-invitation admin CRUD** exposed by recent d6e instance releases. |
...
`skills/<name>/SKILL.md` files in this repository directly from GitHub.
The `owner/repo` shorthand is supported.
